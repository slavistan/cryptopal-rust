...
\sigma^\text{(Correct)}_i &= E[(A \xor A)^i] = p[(A \xor A)^i = 1] \cdot 1 \\
                          &= 2 \cdot p[A^i = 1] \cdot (1 - p[A^i = 1]) \leq 0.5
\end{align}
$$
...
Knowing that $p[(U \xor U)^i = 1] \leq 0.5$ for any byteset $U$ yields the inequality

$$
\begin{align}
\sigma_\text{(False)}  &= p[(A \xor A)^i = 1] \cdot \bigg(1 - p[(K \xor K)^i = 1]\bigg) + \bigg(1 - p[(A \xor A)^i = 1])\bigg) \cdot p[(K \xor K)^i = 1] \\
         &= \sigma^\text{(Correct)}_i + p[(K \xor K)^i = 1] \cdot \underbrace{\bigg(1 - 2 \cdot p[(A \xor A)^i = 1]\bigg)}_{\geq 0} \\
